...
public class Lambda_01_Test {
    ...
    public void test_filter_by_firstname() throws Exception {
        ...
        //result ne doit contenir que des personnes dont le prénom est "first_10"
    ...
    public void test_filter_by_password() throws Exception {
        ...
        //result ne doit contenir que les personnes dont l'age est > 49 et dont le hash du mot de passe correspond à la valeur de la variable passwordSha512Hex
        //Pour obtenir le hash d'un mot, utiliser la méthode DigestUtils.sha512Hex(mot)
        List<Person> result = filter(personList, p -> {
        	String hash = DigestUtils.sha512Hex(p.getPassword());
        	return 	(p.getAge() > 49) && hash.equals(passwordSha512Hex);
        });
